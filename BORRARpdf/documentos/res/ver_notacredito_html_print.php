...
$sumador_total=0;
    ...
    $sql_factura=mysqli_query($$_SESSION['bd_comercial'],"SELECT movimientos.*, clientes.* from movimientos INNER JOIN clientes ON movimientos.id_cliente=clientes.id_cliente WHERE id_movimiento='$idm'");
    $rsql_factura=  mysqli_fetch_array($sql_factura);
    $secuencia=$rsql_factura["numero_factura"];
    $claveacceso=$rsql_factura["claveacceso"];
    $idcliente=$rsql_factura["id_cliente"];
    ...
    //$sql_nc=mysqli_query($$_SESSION['bd_comercial'],"INSERT INTO movimientos (id_movimiento,numero_factura,fecha_movimiento,proceso,id_cliente,iva,conexion)VALUES('$idnc','$ncs','$fechamov','NCR','$idcliente','12','$con')");
    ...
    if(mysqli_num_rows($sql_factura)>0){
        ...
        //$items = trim($items, ';');
        //$rows=explode(";",$items);
        $detallemovo=mysqli_query($$_SESSION['bd_comercial'], "SELECT detalle_movimiento.*, products.codigo_adc, products.nombre_producto, products.IVA from detalle_movimiento INNER JOIN products ON detalle_movimiento.id_producto=products.id_producto WHERE id_movimiento='$idm'");
        while($resdetallemovo=mysqli_fetch_array($detallemovo))
        {
        //for($i=0;$i<count($rows);$i++){
            ...
            //{
                ...
                $descuentog=$resdetallemovo["descuento"];
                ...
                $descuentou=$descuentog/$cantg;
                $descuentou=round($descuentou,2); //descuento unitario
                $descuentototal+=$descuentou;
                $subtotal=$pventag*$cantg;
                $subtotal=round($subtotal,2);
                $subtotalNC+=$subtotal;
                $totalitem=$subtotal-$descuentou;
                $totalitem=round($totalitem,2);
                $pv=number_format($pventag,2,'.','');
                $descuentou=  number_format($descuentou,2,'.','');
                //$insert_detail=mysqli_query($$_SESSION['bd_comercial'], "INSERT INTO detalle_movimiento (id_movimiento, id_producto, cantidad, precio_venta,descuento, iva) VALUES ('".$idnc."','$id_producto','$cantidad','$pv','$descuentou','$iva')");
                //DESCARTAR KARDEX  Y GENERAR TOTALES
                ...
                //$costoventa=$resCPV["salidas_costo"];
                ...
                //$total_mov=number_format($costoventa*$cantidad,4,".",'');
                ...
                //$costoventa= number_format($costoventa,4,'.','');
                ...
                //$subtotal+=$preciot;
                //$insert_kardex=mysqli_query($$_SESSION['bd_comercial'], "INSERT INTO kardex VALUES ('','".$id_producto."','$date2','DEV','$idnc','$cantidad','$costoventa','$total_mov','','','','$nueva_cant','$nuevo_costo','$nuevo_total')");
                ...
                $prod=" ".encabezado($cantg,3)." ".encabezado($codigo_producto,6)." ".encabezado($nombre_producto,35)." ".precios($pv,8)." ".precios($subtotal,7);
                $Cuerpo .= $prod."\n";
	...
	$total_factura=$bimponible0+$bimponible+$total_iva;
        ...
        $Cuerpo .=$inf5."\n";
        $Cuerpo .=$inf6."\n";
        //$sql_n2=mysqli_query($$_SESSION['bd_comercial'],"UPDATE movimientos SET bimponible_iva0='".number_format($bimponible0,2,'.','')."',bimponible_ivax='".number_format($bimponible,2,'.','')."',importe_iva='".number_format($total_iva,2,'.','')."',descuento='".number_format($desc_dolares,2,'.','')."', total_venta='".number_format($total_factura,2,'.','')."',estado_sri='PENDIENTE'  where id_movimiento='$idnc'");
        //$sql_FC=mysqli_query($$_SESSION['bd_comercial'],"UPDATE movimientos SET modifica='$idnc' WHERE id_movimiento='$idm'");
        ...
        $sql_modifica=mysqli_query($$_SESSION['bd_comercial'],"SELECT movimientos.numero_factura from movimientos WHERE modifica='$idm' LIMIT 0,1");
        $rsql_modifica=  mysqli_fetch_array($sql_modifica);
        $docm=$rsql_modifica["numero_factura"];
        $Fin.="Aplica a:".ESTABLECIMIENTO_1."-".PUNTOEMISION_1."-".formato_cadena($docm,9)."\n";
