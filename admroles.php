...
include_once("./modelo/clsroles.php");
...
$rol = new clsroles; // declaro un objeto de la clase de la pagina que gestiono
	...
	switch ($acc){ // evaluamos la accion
		...
		case 2: // modificamos a la base
			   ...
			   $rol->carga_rol_id($reg); // cargo en la clase
			   $rol->carga_rol_nombre($_POST["txtnom"]);
			   $rol->rol_actualizar();
		     break;
        ...
        <div class="table-responsive">
            <table class="table table-hover">
				...
				?>			   
								...
								<thead>
									...
									<tr class="info">
									 ...
									 <?php  
									  ...
									  $rspaireg = $rol->rol_seleccionartodo(); // selecciono todos los registros
									  $registros = mysqli_num_rows($rspaireg);  // selecciono el total de regitros 
									  $rspai = $rol->rol_seleccionarpagina($des,$pagina); // seleccion los siguientes $pagina(10) resitros desde $des(20)
						...
						?>			   
						<div id="secc">
							<div id="titulo2">Ingresar Rol</div>
							<div id="sec_contedor">
						...
						case 2:   ////////   formulario para modificar
						        $reg = get("reg");
								$rol->carga_rol_id($reg);
								$rsrol = $rol->rol_seleccionar();
						        $rolfila = mysqli_fetch_array($rsrol);
						?>	
						<div id="secc">
							<div id="titulo2">Editar Rol</div>
							<div id="sec_contedor">
									<form id="editar" method="post" action="?acc=2&reg=<?php echo $reg?>">  
										<div id="frmcampo"><input class="form-control" value="<?php echo $rolfila[1]?>" name="txtnom" id="txtnom" type="text" autofocus required style="text-transform:uppercase;" onblur="javascript:this.value=this.value.toUpperCase();" >
										</div>
										<div id="frmcampo"><input class='btn btn-default' type="submit" name="button" id="button" value="Guardar" /></div>
									</form>	
