...
Class clstu12respuestas extends clstramiterespuestas{	 
    ...
    public function tuc_actualizar_respuesta($actualizador){
        ...
        //$this->carga_rol_id($bd->lastID()); // sacar el siguiente registro de la tabla y lo cargo en id
        $sql= "update _ct_tramite12_respuestas set tuc_tipocontestacion='".$this->getTuc_tipo_contestacion()."',tuc_rutaarchivo='".$this->getTuc_rutaarchivo()."',tuc_infoadicional='".$this->getInfo_adicional()."', tuc_cumple='".$this->getTuc_cumple()."'".$add." WHERE tuc_id='".$this->getTuc_id()."'";
        //echo $sql;
        //exit();
        $res = $bd->ejecutar($sql);
        //$bd->cerrar();
